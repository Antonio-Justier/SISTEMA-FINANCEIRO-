// Estado fica salvo no navegador (localStorage), então continua disponível
// mesmo depois de fechar e abrir a página de novo — mas só nesse mesmo
// navegador/dispositivo, já que não existe um backend compartilhado aqui.
...
function saveState() {
  try {
    ...
    // Armazenamento indisponível (ex: modo privado do navegador, ou preview
    // restrito dentro de algum app). Os dados continuam funcionando, só não
    // persistem entre recarregamentos nesse caso.
  }
}
...
// Se já havia um salário salvo, mostra ele no campo ao carregar a página.
if (state.salary > 0) {
